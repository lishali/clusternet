...
def GNN(signal_dim = 10, batch_size = 2, SD=1, communities=2, group_size=10, 
    ...
    DATA = [np.asarray(balanced_stochastic_blockmodel(communities, group_size, p, 0.1*p, seed=seed)).astype(np.double) for p in np.linspace(p_min, p_max, Size)]
 ...
 ###################
    ...
    with tf.Session() as sess:
        ...
        for i in xrange(iterations):
            ...
            loss_printed, a, b = sess.run([loss, loss_a, loss_b], 
...
def GNN_seq_2000(v):
    return GNN(Size=400, Mean=v[0], SD=v[1], l_rate=v[2], Mean_signal = v[3], group_size=v[4], batch_size = v[5],
               p_min = v[6], p_max = v[6])
