...
def GNN(signal_dim = 10, batch_size = 2, SD=1, communities=2, group_size=10, 
    p_min = 0.5, p_max = 0.5, Mean = 1, Mean_signal=1, l_rate = 0.0000001, Size=100000):


    """GNN with normed diffusion implemented instead of unnormed version"""
 ...
 ###################
    ...
    train = optimizer.minimize(loss, var_list=[C_a, C_b, B_reduce])
    ...
    with tf.Session() as sess:
        ...
        for i in xrange(iterations):
            ...
            loss_printed = sess.run([loss_a, loss_b, loss], 
            ...
            if i%20==0:
                ...
                d = pd.DataFrame(d)
                d.to_csv("~/Desktop/clusternet/GNN/data/data2/Size{}Mean{}l_rate{}Mean_signal{}group_size{}batch_size{}p_min{}p_max{}.csv".format(Size, Mean, l_rate, Mean_signal, group_size, batch_size, p_min, p_max))
               
            if i==iterations-1:
                print "these are the variables after training:"
                print "mean:", m, "l rate:", l, "Mean_signal:", n, 
                a, b, b_reduce = sess.run([C_a, C_b, B_reduce], feed_dict={Adj: DATA[i:i+batch_size], F: Signal, true_assignment_a: TRUE_A, true_assignment_b: TRUE_B})

                variable_lst = variable_lst+[a, b, b_reduce]
                d_var = {"vars_a_b_ai_bi_b_reduce": variable_lst, "header": ["Ca", "Cb", "B_reduce"]}
                d_var = pd.DataFrame(d_var)
                d_var.to_csv("~/Desktop/clusternet/GNN/data/data2/ModelPARAMS_Size{}Mean{}l_rate{}Mean_signal{}group_size{}batch_size{}p_min{}p_max{}.csv".format(Size, Mean, l_rate, Mean_signal, group_size, batch_size, p_min, p_max))
           
                print d_var

iterated_list = [(m, s, l, n, g, b, p) for m in [0] for 
     s in [1] for 
     l in [10**-i for i in xrange(6, 14, 4)] for 
     n in [0] for 
     g in [20] for
     b in [1, 10, 20] for 
...
def GNN_seq_2000(v):
    return GNN(Size=5000, Mean=v[0], SD=v[1], l_rate=v[2], Mean_signal = v[3], group_size=v[4], batch_size = v[5],
               p_min = v[6], p_max = v[6])
