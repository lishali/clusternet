...
def GNN(signal_dim = 10, batch_size = 2, SD=1, communities=2, group_size=10, 
    p_min = 0.5, p_max = 0.5, Mean = 1, Mean_signal=1, l_rate = 0.0000001, Size=100000, datapoints=10):


    """ First implement of GNN"""
    dim = communities*group_size

    DATA1 = [np.asarray(balanced_stochastic_blockmodel(communities, group_size, p, 0.1*p, seed=None)).astype(np.double) for p in np.linspace(p_min, p_max, datapoints)]
    ...
    with tf.Session() as sess:
        ...
        for i in xrange(iterations):
            ...
            if i%20==0:
                ...
                d = pd.DataFrame(d)
                d.to_csv("~/Desktop/clusternet/GNN/data/dataConstant/DATApoints{}Size{}Mean{}l_rate{}Mean_signal{}group_size{}batch_size{}p_min{}p_max{}.csv".format(datapoints, Size, Mean, l_rate, Mean_signal, group_size, batch_size, p_min, p_max))
            ...
            if i==iterations-1:
                ...
                d_var.to_csv("~/Desktop/clusternet/GNN/data/dataConstant/ModelPARAMS_DATApoints{}Size{}Mean{}l_rate{}Mean_signal{}group_size{}batch_size{}p_min{}p_max{}.csv".format(datapoints, Size, Mean, l_rate, Mean_signal, group_size, batch_size, p_min, p_max))
           
                #print d_var

iterated_list = [(m, s, l, n, g, b, p, d) for m in [0] for 
     s in [1] for 
     l in [10**-i for i in [3, 5, 7]] for #xrange(6, 14, 4)] for 
     n in [0] for 
     g in [20] for
     b in [1] for 
     p in [0.3, 0.4, 0.5] for 
     d in [2, 4, 8]]

k = np.random.randint(1, 1000)

def GNN_seq_2000(v):
    return GNN(Size=20, Mean=v[0], SD=v[1], l_rate=v[2], Mean_signal = v[3], group_size=v[4], batch_size = v[5],
               p_min = v[6], p_max = v[6], datapoints=v[7])
